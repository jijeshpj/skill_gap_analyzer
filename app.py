...
def load_nlp_model():
    ...
    try:
        ...
        # 2. ലോഡ് പരാജയപ്പെട്ടാൽ, Streamlit-ൽ വെച്ച് ഡൗൺലോഡ് ചെയ്യുക
        st.info("SpaCy model 'en_core_web_sm' not found. Attempting download on the server...")
        try:
            # pip കമാൻഡ് ഉപയോഗിച്ച് ഡൗൺലോഡ് ചെയ്യുന്നു
            subprocess.check_call(["python", "-m", "spacy", "download", "en_core_web_sm"])
            
            # 3. വിജയകരമായി ഡൗൺലോഡ് ചെയ്ത ശേഷം വീണ്ടും ലോഡ് ചെയ്യുക
            nlp = spacy.load("en_core_web_sm")
            st.success("SpaCy model successfully downloaded and loaded!")
            return nlp
        except subprocess.CalledProcessError:
            st.error("❌ Critical Error: Failed to download and load the spaCy model 'en_core_web_sm'.")
            return None
