...
def load_nlp_model():
    ...
    try:
        # റിപ്പോസിറ്ററിയിൽ നിന്ന് നേരിട്ട് ലോഡ് ചെയ്യാൻ ശ്രമിക്കുന്നു (ഇത് GitHub-ൽ ഫോൾഡർ ഉണ്ടെങ്കിൽ പ്രവർത്തിക്കും)
        # Streamlit, നിങ്ങളുടെ ആപ്ലിക്കേഷൻ ഫോൾഡറിനെയാണ് തിരയുന്നത്.
        nlp = spacy.load("en_core_web_sm") 
        st.success("SpaCy model loaded successfully from repository!") 
        return nlp
    except OSError as e:
        # മോഡൽ ലോഡ് ചെയ്യാൻ സാധിച്ചില്ലെങ്കിൽ പിശക് സന്ദേശം നൽകുന്നു.
        st.error(f"❌ Critical Error: SpaCy model 'en_core_web_sm' not found in the repository path. Please check the folder exists in GitHub. Error: {e}")
        return None
