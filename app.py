...
@st.cache_resource 
def load_nlp_model():
    """SpaCy മോഡൽ ഇല്ലെങ്കിൽ pip ഉപയോഗിച്ച് ഡൗൺലോഡ് ചെയ്ത് ലിങ്ക് ചെയ്ത് ലോഡ് ചെയ്യുന്നു."""
    try:
        # 1. ലോഡ് ചെയ്യാൻ ശ്രമിക്കുക
        nlp = spacy.load("en_core_web_sm") 
        st.success("SpaCy model loaded successfully.") 
        return nlp
    except OSError:
        # 2. ലോഡ് പരാജയപ്പെട്ടാൽ, pip ഉപയോഗിച്ച് ഡൗൺലോഡ് ചെയ്യാൻ ശ്രമിക്കുക
        st.info("SpaCy model 'en_core_web_sm' not found. Attempting installation and linking via pip...")
        
        try:
            # 🚨 പരിഹാരം: pip ഉപയോഗിച്ച് ഡൗൺലോഡ് ചെയ്യുന്നു. 🚨
            ...
            # 🚨 പുതിയത്: ഇൻസ്റ്റാൾ ചെയ്ത ശേഷം മോഡലിനെ ലിങ്ക് ചെയ്യുന്നു. 🚨
            # ഇത് `spacy.load()` കമാൻഡിന് മോഡലിനെ എവിടെ നിന്ന് കണ്ടെത്തണമെന്ന് പറഞ്ഞു കൊടുക്കുന്നു.
            subprocess.check_call(["python", "-m", "spacy", "link", "en_core_web_sm", "en_core_web_sm", "--force"])
            
            # 3. ഡൗൺലോഡ് & ലിങ്കിംഗ് വിജയകരമായാൽ, മോഡലിനെ വീണ്ടും ലോഡ് ചെയ്യുക
            nlp = spacy.load("en_core_web_sm")
            st.success("SpaCy model successfully installed, linked, and loaded!")
            return nlp
            
        except subprocess.CalledProcessError as e:
            # ഡൗൺലോഡ് / ലിങ്കിംഗ് പരാജയപ്പെട്ടാൽ പിശക് സന്ദേശം നൽകുന്നു
            st.error(f"❌ Critical Error: Failed to install, link, or load the spaCy model. Error: {e}")
            return None
