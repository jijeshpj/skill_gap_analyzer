...
def load_nlp_model():
    ...
    try:
        ...
        try:
            # 🚨 പരിഹാരം: pip കമാൻഡ് ഉപയോഗിച്ച് ഡൗൺലോഡ് ചെയ്യുന്നു. 🚨
            # ഇത് `spacy download` കമാൻഡിനേക്കാൾ Streamlit-ൽ സ്ഥിരത നൽകും.
            ...
            # ഡൗൺലോഡ് വിജയകരമായാൽ, മോഡലിനെ വീണ്ടും ലോഡ് ചെയ്യുക
            nlp = spacy.load("en_core_web_sm")
            st.success("SpaCy model successfully downloaded and loaded!")
            return nlp
            
        except subprocess.CalledProcessError as e:
            # ഡൗൺലോഡ് പരാജയപ്പെട്ടാൽ പിശക് സന്ദേശം നൽകുന്നു
            st.error(f"❌ Critical Error: Failed to install and load the spaCy model. Error: {e}")
            return None
