...
import subprocess # 🚨 പുതിയതായി ചേർക്കുക!
...
@st.cache_resource 
def load_nlp_model():
    """SpaCy മോഡൽ ഇല്ലെങ്കിൽ pip ഉപയോഗിച്ച് ഡൗൺലോഡ് ചെയ്ത് ലോഡ് ചെയ്യുന്നു."""
    try:
        # 1. ലോഡ് ചെയ്യാൻ ശ്രമിക്കുക (ഇതാണ് ഇപ്പോൾ പരാജയപ്പെടുന്നത്)
        nlp = spacy.load("en_core_web_sm") 
        st.success("SpaCy model loaded successfully.") 
        return nlp
    except OSError:
        # 2. ലോഡ് പരാജയപ്പെട്ടാൽ, pip ഉപയോഗിച്ച് ഡൗൺലോഡ് ചെയ്യാൻ ശ്രമിക്കുക
        st.info("SpaCy model 'en_core_web_sm' not found. Attempting installation via pip...")
        
        try:
            # 🚨 പരിഹാരം 1: pip ഉപയോഗിച്ച് ഡൗൺലോഡ് ചെയ്യുന്നു (ലിങ്ക് ഒഴിവാക്കി) 🚨
            # ഇത് മോഡലിനെ Python package ആയി ഇൻസ്റ്റാൾ ചെയ്യും.
            subprocess.check_call(["pip", "install", "https://github.com/explosion/spacy-models/releases/download/en_core_web_sm-3.7.1/en_core_web_sm-3.7.1.tar.gz"])
            
            # 🚨 പരിഹാരം 2: ലോഡ് ചെയ്യുമ്പോൾ നേരിട്ട് പാക്കേജ് നെയിം ഉപയോഗിക്കുക 🚨
            # Symlink ഇല്ലാത്തതുകൊണ്ട്, load ചെയ്യുമ്പോൾ പാക്കേജ് നെയിം ഉപയോഗിക്കാം.
            # പാക്കേജ് നെയിം മോഡലിൻ്റെ പേരിന് സമാനമായതുകൊണ്ട്, ഇത് പ്രവർത്തിക്കാൻ സാധ്യതയുണ്ട്.
            nlp = spacy.load("en_core_web_sm") 
            st.success("SpaCy model successfully installed and loaded as a package!")
            return nlp
            
        except subprocess.CalledProcessError as e:
            # ഇൻസ്റ്റാളേഷൻ പരാജയപ്പെട്ടാൽ പിശക് സന്ദേശം നൽകുന്നു
            st.error(f"❌ Critical Error: Failed to install or load the spaCy model. Error: {e}")
            return None

nlp = load_nlp_model() # മോഡൽ ലോഡ് ചെയ്യുന്നു

# ... (ബാക്കി കോഡുകൾ മാറ്റമില്ല) ...
