...
@st.cache_resource 
def load_nlp_model():
    """SpaCy മോഡൽ ഇല്ലെങ്കിൽ ഡൗൺലോഡ് ചെയ്ത് ലോഡ് ചെയ്യുന്നു."""
    try:
        # 1. റിപ്പോസിറ്ററിയിൽ നിന്ന് നേരിട്ട് ലോഡ് ചെയ്യാൻ ശ്രമിക്കുക
        nlp = spacy.load("en_core_web_sm") 
        st.success("SpaCy model loaded successfully.") # വിജയിച്ചാൽ സന്ദേശം നൽകാം
        return nlp
    except OSError:
        # 2. ലോഡ് പരാജയപ്പെട്ടാൽ, Streamlit-ൽ വെച്ച് ഡൗൺലോഡ് ചെയ്യാൻ ശ്രമിക്കുക
        st.info("SpaCy model 'en_core_web_sm' not found. Attempting download on the server...")
        
        try:
            # subprocess വഴി മോഡൽ ഡൗൺലോഡ് ചെയ്യുക (pip install ചെയ്യുന്നതിന് തുല്യം)
            ...
            st.error(f"❌ Critical Error: Failed to download and load the spaCy model. Error: {e}")
            return None
